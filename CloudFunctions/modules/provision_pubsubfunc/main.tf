...
resource "google_cloudfunctions_function_iam_member" "pubsubinvoker" {
  project        = var.project_id
  region         = var.region
  cloud_function = google_cloudfunctions_function.pubsubfunction.name

  role   = "roles/cloudfunctions.invoker"
  member = "serviceAccount:${google_service_account.cloud_function_service_account.email}"

  depends_on = [
    google_cloudfunctions_function.pubsubfunction
  ]
}

resource "google_cloudfunctions_function" "pubsubfunction" {
