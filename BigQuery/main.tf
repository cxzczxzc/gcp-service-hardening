...
resource "google_bigquery_table_iam_binding" "materialized_view_binding" {
 project    = google_bigquery_table.materialized_view.project_id
  ...
  dynamic "materialized_view_access" {
    for_each = var.materialized_view_access
    content {
      role           = lookup(local.iam_to_primitive, materialized_view_access.value.role, materialized_view_access.value.role)
      domain         = lookup(materialized_view_access.value, "domain", "")
      group_by_email = lookup(materialized_view_access.value, "group_by_email", "")
      user_by_email  = lookup(materialized_view_access.value, "user_by_email", "")
      special_group  = lookup(materialized_view_access.value, "special_group", "")
    }
  }
}
*/
...
resource "google_bigquery_table_iam_binding" "external_table_binding" {
  ...
  table_id   = google_bigquery_table.external_table.table_id

  dynamic "external_table_access" {
    for_each = var.external_table_access
    content {
      role           = lookup(local.iam_to_primitive, external_table_access.value.role, external_table_access.value.role)
      domain         = lookup(external_table_access.value, "domain", "")
      group_by_email = lookup(external_table_access.value, "group_by_email", "")
      user_by_email  = lookup(external_table_access.value, "user_by_email", "")
      special_group  = lookup(external_table_access.value, "special_group", "")
    }
  }
}
*/
