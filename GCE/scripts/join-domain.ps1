...
Wrap {
    ...
    if ((gwmi win32_computersystem).partofdomain -eq $false)
    {
        ...
        $username = ($service_object | Select -ExpandProperty UserName) + '@dnbapp.net'

        # Create a PSCredential object
        $Credential = New-Object System.Management.Automation.PSCredential -ArgumentList $username, $( ConvertTo-SecureString ($service_object | Select -ExpandProperty Content) -AsPlainText -Force )

        # Join the domain
        Add-Computer -DomainName ($service_object | Select -ExpandProperty LogonDomain) -Credential $Credential -OUPath $ouPath -PassThru -Restart
    } else {
